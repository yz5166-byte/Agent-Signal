...
def fetch(per_query: int = PER_QUERY) -> list[Item]:
    ...
    两路的 star 数不可比——老项目有几年时间累积，新项目只有几十天。
    所以把路径写进 source 字段，让 curate 能给两路各留名额，
    否则按 star 排序时经典路会把新星路整个挤掉（实测 14:1）。
    两路可能返回重复项目，不在这里去重——去重是 curate 的职责。
    """
    tracks = {
        # 新星：90 天内新建、已有 50+ star，聚焦 Agent
        "GitHub 新星": f"agent in:name,description created:>{_days_ago(90)} stars:>50",
